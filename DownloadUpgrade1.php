<?php
	...
	if ($bldg_id <> '' || $bldg_name <> '' ) {
		...
		$existing_query = mysql_query("SELECT * FROM bugged_locations WHERE owner_id='$id' AND d1=0 AND building_id='$bldg_id'") or die(mysql_error());
		$player_query1 = mysql_query("SELECT * FROM player_sheet WHERE id='$id'") or die(mysql_error());
		
		if ( mysql_num_rows($player_query1)==1) {
			//we have found our player entry
			$player_data1 = mysql_fetch_assoc($player_query1);
			$intel = $player_data1['intel'];
			$cost = 1000000; //points are in 'bytes'- this is 1GB

			if ($intel >= $cost) {
				$intel = $intel - $cost;
				$bldg_query = "";
				if(mysql_num_rows($existing_query)==0){
					$existing_query1 = mysql_query("SELECT * FROM bugged_locations WHERE owner_id='$id' AND d1=1 AND building_id='$bldg_id'") or die(mysql_error());
					if (mysql_num_rows($existing_query1)==0){
						$bldg_query ="INSERT INTO bugged_locations SET owner_id='$id', bldg_name='$bldg_name', bldg_id='$bldg_id', d1=1";
					}else{
						$bldg_query="UPDATE bugged_locations SET d1=1 WHERE owner_id='$id' AND building_id='$bldg_id'";
					}
				}else{
					$bldg_query="UPDATE bugged_locations SET d1=1 WHERE owner_id='$id' AND building_id='$bldg_id'";
				}
