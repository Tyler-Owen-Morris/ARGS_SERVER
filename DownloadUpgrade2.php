<?php
	...
	if ($bldg_id <> '' || $bldg_name <> '' ) {
		...
		$existing_query = mysql_query("SELECT * FROM bugged_locations WHERE owner_id='$id' AND d2=0 AND building_id='$bldg_id'") or die(mysql_error());
	...
	//		$cost = 1000000000; //points are in 'bytes'- this is 1TB
			...
			if ($intel >= $cost) {
				...
				if(mysql_num_rows($existing_query)==0){
					$existing_query1 = mysql_query("SELECT * FROM bugged_locations WHERE owner_id='$id' AND d2=1 AND building_id='$bldg_id'") or die(mysql_error());
					if (mysql_num_rows($existing_query1)==0){
						$bldg_query ="INSERT INTO bugged_locations SET owner_id='$id', bldg_name='$bldg_name', bldg_id='$bldg_id', d2=1";
					}else{
						$bldg_query="UPDATE bugged_locations SET d2=1 WHERE owner_id='$id' AND building_id='$bldg_id'";
					}
				}else{
					$bldg_query="UPDATE bugged_locations SET d2=1 WHERE owner_id='$id' AND building_id='$bldg_id'";
				}
