<?php
	...
	if ($bldg_id <> '' || $bldg_name <> '' || $cost <> '') {
		...
		if (mysql_num_rows($existing_query)> 0) {
			$update_query = mysql_query("UPDATE bugged_locations SET t3_bug_count=t3_bug_count+1 WHERE owner_id='$id' AND building_id='$bldg_id'") or die(mysql_error());
		}else{
			$insert_query = mysql_query("INSERT INTO bugged_locations SET owner_id='$id', building_name='$bldg_name', building_id='$bldg_id', t3_bug_count=1, last_download_ts= now()") or die(mysql_error());
		}
